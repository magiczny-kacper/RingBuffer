...
TEST(ring_tests, init_with_null)
{
   RingBuffer_t myRing;
   uint8_t buf[256];
   TEST_ASSERT_EQUAL(NO_PTR, RingInit(&myRing, NULL, 256, sizeof(uint8_t)));
   TEST_ASSERT_EQUAL(NO_PTR, RingInit(NULL, &buf[0], 256, sizeof(uint8_t)));
}
...
TEST(ring_tests, init_with_no_size)
{
   RingBuffer_t myRing;
   uint8_t arr[10];
   TEST_ASSERT_EQUAL(NO_DATA, RingInit(&myRing, &arr[0], 0, sizeof(uint8_t)));
}
...
TEST(ring_tests, read_byte_after_init)
{
   ...
   RingInit(&myRing, &arr[0], 10, sizeof(uint8_t));
   TEST_ASSERT_EQUAL(NO_DATA, RingReadElement(&myRing, &data));
}
...
TEST(ring_tests, read_bytes_to_empty)
{
   ...
   for(uint8_t i = 0; i < sizeNum; i++){
      TEST_ASSERT_EQUAL(OK, RingReadElement(&myRing, &data));
