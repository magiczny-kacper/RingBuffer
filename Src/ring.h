...
typedef enum {
    ...
    NO_PLACE = -1, /**< Returned if ther's no place to write data in buffer. */
 ...
 * @return RingStatus_t
 */
RingStatus_t RingInit(RingBuffer_t* buffer, void* arrayBuffer,
        size_t bufferSize, size_t elementSize);
 ...
 * @return RingStatus_t
 */
RingStatus_t RingInitAlloc(RingBuffer_t* buffer, size_t bufferSize,
        size_t elementSize);
